...
export const getAllIndexPoints = async (req, res) => {
  ...
  if (isMarketTime()) {
    // Market is open: Use current day for currentDateObj
    currentDateObj = now;
    previousDateObj = getLastTradingDay(now);
  } else {
    // Market is closed: Use last trading day as currentDateObj
    currentDateObj = getLastTradingDay(now);
    previousDateObj = getLastTradingDay(currentDateObj);
  }
  ...
  const currentTimestamp = `${currentDateStr}, 03:27:00 PM`;
  const previousTimestamp = `${previousDateStr}, 03:27:00 PM`;
