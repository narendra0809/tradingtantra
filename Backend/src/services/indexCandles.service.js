...
const mergeCandles = (
  ...
  // Add after-market close to 3:27 PM candle (3m interval)
  if (afterMarketClose !== null) {
    const targetTimestamp = moment
      .tz(tradingDay, "DD-MM-YYYY", "Asia/Kolkata")
      .set({ hour: 15, minute: 27, second: 0, millisecond: 0 })
    ...
    console.log("Target Timestamp (3m after-market): ", targetTimestamp);
...
// Fetch data from Dhan API for 3m (using 1m candles)
  ...
  try {
    const response = await axios.post(
      DHAN_API_URL,
      {
        ...
        exchangeSegment: index.seg,
        instrument: "INDEX",
        interval: "1", // 1-minute candles from Dhan
        oi: false,
        fromDate: formattedFromDate,
        toDate: formattedToDate,
      },
    ...
    if (error.response?.status === 429) {
      ...
      await delay(150);
      return fetchDhanData(index, fromDate, toDate);
    }
    console.error(
      `Error fetching 1m data for ${index.name}:`,
      error.response?.data || error.message
    );
    return null;
  }
...
// Process and save ONLY 3m candles for an index
const processIndexCandles = async (index, apiData, currentTime, tradingDay) => {
  if (!apiData) {
    console.log(`No data to process for ${index.name} (3m)`);
    return;
  }

  try {
    const interval = "3m";
    const candles = mergeCandles(apiData, 3, currentTime, index.name, tradingDay);
    ...
    console.error(`Error processing ${index.name} candles (3m):`, error);
  }
};

// Fetch and process data for all indices (ONLY 3m from 1m)
const fetchAndProcessAllIndices = async (fromDate, toDate) => {
  const currentTime = new Date();
  ...
  for (const index of indices) {
    ...
    if (apiData) {
      await processIndexCandles(index, apiData, currentTime, toDate);
    }
    await delay(150);
  }

  console.log("All indices processed successfully (3m only)");
};
...
export const runFetchForIndexCandles = async () => {
  try {
    const today = moment().tz("Asia/Kolkata");
    const currentDate = formatDateForAPI(today); // DD-MM-YYYY
    ...
    // Only fetch and process current day's data (3m derived from 1m)
    await fetchAndProcessAllIndices(currentDate, currentDate);
  } catch (error) {
    console.error("Error in runFetchForIndexCandles:", error);
    throw error;
