...
const getData = async () => {
  ...
  try {
    ...
    for (let i = 0; i < securityIds.length; i++) {
      ...
      if (completeCandles.length >= 12) {
        ...
        try {
          ...
          //LOG  // console.error(`[MongoDB] Error saving 5-min data for ${id}: ${error.message}`);
        }
        ...
        if (tenMinCandles) {
          ...
          try {
            ...
            console
              .error
              //LOG  // `[MongoDB] Error saving 10-min data for ${id}: ${error.message}`
              ();
          }
        }
      }
    ...
    for (let i = 0; i < securityIds.length; i++) {
      ...
      if (completeCandles.length >= 5) {
        ...
        try {
          ...
          console
            .error
            //LOG  `[MongoDB] Error saving 15-min data for ${id}: ${error.message}`
            ();
        }
      }
    ...
    //LOG // console.error("[Main] Error in getData:", error.message);
    throw error;
  }
};
...
const AIIntradayReversalFiveMins = async (req, res) => {
  try {
    ...
    if (finalResults.length) {
      ...
      console.log(
        `[MongoDB] Reversal signals saved or updated for IntradayReversalFiveMin`
      );
    }
    ...
    console.error(
      `[Main] Error in AIIntradayReversalFiveMins: ${error.message}`
    );
    return {
      message: "Internal server error",
      error: error.message,
