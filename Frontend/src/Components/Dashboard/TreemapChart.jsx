...
//   if (change > 0)
//     return `rgba(34, 177, 76, ${Math.min(change / 10 + 0.3, 1)})`;
//   if (change < 0)
...
const getColor = (change) => {
  ...
  if (change < 0) return `rgba(220, 50, 50, ${Math.min(Math.abs(change) / 10 + 0.3, 1)})`;
  return "rgba(169, 169, 169, 0.8)";
};

const TreemapChartDynamic = ({data}) => {
  const [tooltip, setTooltip] = useState(null);

  // Transform data as Recharts expects
  const treemapData = data.map(item => ({
    name: item.UNDERLYING_SYMBOL,
    size: item.xelement,        // this determines box area
    change: item.percentageChange ?? 0,
  }));
